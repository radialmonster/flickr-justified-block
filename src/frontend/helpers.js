...
export function createLoadingIndicatorElement( baseLoadingMessage ) {
...
export function maintainLoadingIndicator( options ) {
	const {
		gallery,
		loadingIndicator,
		baseLoadingMessage,
	...
	if ( shouldPersist ) {
		...
		indicator.textContent =
			indicatorMessage || indicator.textContent || baseLoadingMessage;
...
export function getLoadedCount( gallery ) {
	return parseInt( gallery.getAttribute( 'data-loaded-count' ) || '0', 10 );
}

export function setLoadedCount( gallery, count ) {
	gallery.setAttribute( 'data-loaded-count', count );
}

export function getPhotoLimit( gallery ) {
