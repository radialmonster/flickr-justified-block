/**
 * Flickr Justified Gallery - Lazy Loading Module (ES Module)
 *
...
function destroyGallery(gallery) {
    ...
    log('Gallery destroyed:', gallery.id);
}

/**
 * Returns the imported helper functions as a bundle.
 */
function getHelpersOrThrow() {
    return { getLoadedCount, setLoadedCount, getPhotoLimit, createLoadingIndicatorElement, maintainLoadingIndicator };
}
...
function initGallery(gallery) {
    ...
    if (existingState && existingState.initialized) {
        log('Gallery already initialized, skipping:', gallery.id);
        return;
    }

    log(`Initializing lazy loading for gallery:`, gallery.id);
    ...
    try {
        ...
        log(`Loaded set metadata for ${state.setMetadata.length} photosets`);
        ...
        if (observerSetup) {
            ...
            log(`Gallery ${gallery.id} fully initialized`);
        } else {
            log(`Gallery ${gallery.id} setup incomplete - waiting for cards to arrive`);
            ...
            if (!state.mutationObserver) {
                        ...
                        if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                            ...
                            for (const node of mutation.addedNodes) {
                                if (node.nodeType === Node.ELEMENT_NODE &&
                                    ...
                                    log(`Cards detected in ${gallery.id}, retrying initialization`);
                ...
                state.mutationObserverTimeout = setTimeout(() => {
                    if (state.mutationObserver) {
                        warn(`Gallery ${gallery.id}: No cards arrived after 30s, stopping MutationObserver`);
                        state.mutationObserver.disconnect();
                        state.mutationObserver = null;
                        state.mutationObserverTimeout = null;
                    }
                ...
                log(`MutationObserver set up for ${gallery.id} (30s timeout)`);
            } else {
                log(`MutationObserver already watching ${gallery.id}`);
            }
        }
...
function setupIntersectionObserver(gallery, state) {
    ...
    if (!lastImage) {
        log('No last image found for intersection observer - cannot set up lazy loading');
        return false;
    }
    ...
    state.io = new IntersectionObserver((entries) => {
        ...
        for (const entry of entries) {
            ...
            log('Last image visible - triggering loadNextPages');
            loadNextPages(gallery);
        }
    }, { rootMargin: '500px' });

    state.io.observe(lastImage);
    log('Observing last image for lazy loading');
    return true;
}
...
function initAllGalleries(root = document) {
    ...
    if (galleriesWithSets.length === 0) {
        log('No galleries with set metadata found');
        return;
    }

    log(`Setting up lazy loading for ${galleriesWithSets.length} galleries`);
...
async function loadNextPages(gallery) {
    ...
    if (photoLimit > 0 && loadedBefore >= photoLimit) {
        log('Photo limit reached, stopping lazy loading');
        stopLazyLoading(state);
        return;
    }
    ...
    if (pendingSets.length === 0) {
        log('All pages loaded or errored');
        stopLazyLoading(state);
        return;
    }
    ...
    function reinitializeGallery() {
        log('Reinitializing gallery layout...');
        ...
        log('Gallery reinitialization complete');
    }
    ...
    log(`Loading next pages for ${pendingSets.length} sets (request #${thisRequestId})`);
    ...
    try {
        ...
        if (thisRequestId !== state.lastRequestId) {
            log(`Request #${thisRequestId} superseded by #${state.lastRequestId}, ignoring`);
            return;
        }
        ...
        for (const setData of pendingSets) {
            ...
            if (photoLimit > 0) {
                ...
                if (currentLoaded >= photoLimit) {
                    log(`Photo limit reached (${currentLoaded}/${photoLimit}), stopping lazy loading`);
            ...
            if (thisRequestId !== state.lastRequestId) {
                log(`Request #${thisRequestId} superseded during sequential fetch`);
                break;
            }
            ...
            if (result?.noRetry) {
                log(`Non-retryable error from set ${setData.photoset_id}, stopping fetch loop`);
                break;
            }
        }
        ...
        if (thisRequestId !== state.lastRequestId) {
            log(`Request #${thisRequestId} superseded, ignoring results`);
            return;
        }
        ...
        if (noRetryResult) {
            ...
            indicatorMessage = noRetryResult.message || 'Authorization expired. Please refresh the page.';
            ...
            log(`Non-retryable error: ${noRetryResult.error} - user must refresh`);
        } else {
            if (allCancelled) {
            ...
            if (!state.hasMore && !skipHasMoreRecalc) {
                stopLazyLoadingDeferFlags(state);
                log('Disconnected observer - no more pages to load');
            }
            ...
            if (loadedNewPhotos) {
                ...
                log('Reinitializing gallery with new photos...');
                ...
                log('Load succeeded but no new photos returned');
            }
            ...
            if (skipHasMoreRecalc && photoLimit > 0 && loadedBefore + state.pendingPhotos.length >= photoLimit) {
                ...
                log('Photo limit reached, lazy loading stopped');
                ...
                indicatorMessage = firstRecoverableMessage || 'Temporary issue loading images. Retrying shortly...';
                scheduledRetryDelay = backoffDelay;

                log(`Recoverable error, will retry in ${backoffDelay}ms (exponential: ${exponentialDelay}ms, suggested: ${suggestedDelay}ms, attempt ${state.failCount})`);
        ...
        if (error.name === 'AbortError') {
            log('Request aborted, not counting as failure');
        ...
        if (stillExpectingPages) {
            ...
            indicatorShouldPersist = true;
            indicatorMessage = 'Temporary issue loading images. Retrying shortly...';

            log(`Error, will retry in ${backoffDelay}ms (attempt ${state.failCount})`);
        } else {
            if (state.retryTimer) {
                clearTimeout(state.retryTimer);
                state.retryTimer = null;
...
async function loadSetPage(gallery, setData, state, thisRequestId, photoLimit, loadedBefore) {
    ...
    log(`Loading page ${nextPage} for set ${setData.photoset_id}`);

    try {
        // Check if request is still current before starting
        if (thisRequestId !== state.lastRequestId) {
            log(`Request superseded, skipping fetch`);
            return { status: 'cancelled' };
        }

        // Get REST API URL from script module data (required - no hardcoded fallback)
        ...
        const response = await fetch(restUrl + '/load-album-page', {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            if (response.status === 401 || response.status === 403) {
                return {
                    ...
                    retryDelay: 0,
                    noRetry: true,
                    message: 'Authorization expired. Please refresh the page.'
                };
            }
            ...
            if (response.status >= 500 && response.status < 600) {
                return {
                    ...
                    error: `HTTP ${response.status}`,
                    retryDelay: 5000,
                    message: 'Server error. Retrying shortly...'
                };
            }
        ...
        if (!data || !Array.isArray(data.photos)) {
            return {
                ...
                retryDelay: 5000,
                message: 'Bad response payload. Retrying shortly...'
            };
        }
        ...
        if (data.photos.length > 0) {
            ...
            for (const photo of data.photos) {
                ...
                if (state.renderedPhotoIds.has(photoId)) {
                    log(`Skipping duplicate photo: ${photoId}`);
                    continue;
                }
            ...
            if (photoLimit > 0) {
                ...
                if (remaining <= 0) {
                    ...
                    log(`Clamped ${candidates.length} candidates to ${remaining} to respect photoLimit=${photoLimit} (loaded: ${alreadyLoaded})`);
                }
            }
            ...
            if (state.renderedPhotoQueue.length === MAX_RENDERED_IDS) {
                log(`De-dupe at capacity: tracking ${MAX_RENDERED_IDS} most recent photo IDs`);
            }

            log(`Loaded ${data.photos.length} photos (${candidates.length} unique, ${accepted.length} accepted) from page ${nextPage}`);
            ...
            if (nextPage >= setData.total_pages || !data.has_more) {
                log(`Reached last page (${nextPage}/${setData.total_pages}) for set ${setData.photoset_id}`);
            }

            return { status: 'success', photos: acceptedPhotos };
        } else {
            log(`No more photos returned for page ${nextPage}`);
            // Update state in memory (no DOM write needed!)
        ...
        if (error.name === 'AbortError') {
            log(`Request aborted for set ${setData.photoset_id}`);
            return { status: 'cancelled' };
        }
        ...
        if (isNetworkish || isJsonParse) {
            return {
                ...
                retryDelay: 5000,
                message: isJsonParse
                    ? 'Bad JSON response. Retrying shortly...'
                    : 'Network issue. Retrying shortly...'
            };
        }
...
function createPhotoCard(photoData, gallery) {
    ...
    anchor.setAttribute('aria-label', 'View photo: ' + captionText);
...
export { initFlickrAlbumLazyLoading, initAllGalleries, destroyGallery };
...
log('Flickr Justified Gallery: Lazy loading module loaded');
