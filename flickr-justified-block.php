...
class FlickrJustifiedBlock {
    ...
    public static function init() {
        ...
        add_filter('script_module_data_flickr-justified-block-view-script-module', [__CLASS__, 'get_script_module_data']);
     ...
     *
     * The editor script is auto-registered by block.json's editorScript field.
     * WP generates the handle "flickr-justified-block-editor-script".
     * We only need to attach the inline metadata (admin-configured defaults)
     * and translations.
     */
    public static function enqueue_editor_assets() {
        // Inject only admin-configured attribute defaults that differ from
        // block.json's static values. The editor JS applies these on top of
        // the imported block.json metadata.
        $handle = 'flickr-justified-block-editor-script';
        $configured_defaults = [];
        ...
        if (!empty($configured_defaults)) {
            wp_add_inline_script(
                $handle,
                'window.flickrJustifiedDefaults = ' . wp_json_encode([
                    'responsiveSettings' => $configured_defaults,
                ]) . ';',
                'before'
            );
        }
    ...
    public static function enqueue_block_assets() {
        ...
        if ($style_ver) {
            // block.json style registration doesn't use filemtime — override
            // the version on the auto-registered handle so browsers fetch
            // updated CSS after file changes.
            global $wp_styles;
            if (isset($wp_styles->registered['flickr-justified-block-style'])) {
                $wp_styles->registered['flickr-justified-block-style']->ver = (string) $style_ver;
            }
        }

        // Frontend JS is auto-enqueued via block.json "viewScriptModule".
        // Server data is provided via the script_module_data filter.
    }

    /**
     * Provide server-side data to the frontend view script module.
     *
     * WordPress 6.7+ outputs this as a JSON <script> tag that the
     * ES module reads via document.getElementById().
     *
     * @param array $data Existing module data.
     * @return array Merged data with plugin config.
     */
    public static function get_script_module_data($data) {
        return array_merge($data, [
            'pluginUrl'     => FLICKR_JUSTIFIED_PLUGIN_URL,
            'ajaxurl'       => admin_url('admin-ajax.php'),
            'fallbackNonce' => wp_create_nonce('flickr_justified_refresh'),
            'asyncNonce'    => wp_create_nonce('flickr_justified_async_load'),
