...
class FlickrJustifiedCache {
    ...
    public static function get_photo_sizes($photo_id, $page_url, $requested_sizes = ['large', 'original'], $needs_metadata = false, $force_refresh = false) {
        ...
        if (!$force_refresh) {
            ...
            // Fallback: try the comprehensive sizes key (album API path stores data under this key)
            $comprehensive_sizes = self::get_comprehensive_size_list();
            $comprehensive_key_hash = md5(implode(',', $comprehensive_sizes));
            if ($comprehensive_key_hash !== $requested_sizes_key) {
                $local_cache = self::get_photo_cache_locally($photo_id, $comprehensive_key_hash);
                if (is_array($local_cache) && !empty($local_cache) && !isset($local_cache['not_found'])) {
                    // Populate transient cache for next time
                    self::set($base_cache_key, $local_cache);
                    return $local_cache;
                }
    ...
    /**
     * Fallback: reconstruct album photo list from the local membership table.
     *
     * Used when the Flickr API is unavailable (e.g. missing API key after migration)
     * but album data was previously cached in the durable wp_fjb_photo_membership table.
     *
     * @param string   $photoset_id Album/set ID.
     * @param string   $username    Username or path alias from the album URL (used to build photo URLs).
     * @param int      $page        Page number (1-based).
     * @param int      $per_page    Photos per page.
     * @return array|null Photoset result array or null when no local data exists.
     */
    public static function get_photoset_photos_locally($photoset_id, $username, $page = 1, $per_page = 500) {
        $photoset_id = trim((string) $photoset_id);
        if ('' === $photoset_id || '' === $username) {
            return null;
        }
        ...
        $total = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$membership_table} WHERE photoset_id = %s",
            $photoset_id
        ));
        ...
        $photo_ids = $wpdb->get_col($wpdb->prepare(
            "SELECT photo_id FROM {$membership_table} WHERE photoset_id = %s ORDER BY position ASC LIMIT %d OFFSET %d",
            $photoset_id,
            $per_page,
            $offset
        ));
        ...
        foreach ($photo_ids as $pid) {
            $photo_url = 'https://flickr.com/photos/' . rawurlencode($username) . '/' . $pid . '/';
            $photos[] = $photo_url;
            $photo_urls_map[$pid] = $photo_url;

            $views = $wpdb->get_var($wpdb->prepare(
                "SELECT views FROM {$meta_table} WHERE photo_id = %s",
                $pid
            ));
            if (null !== $views) {
                $photo_views[$pid] = (int) $views;
            }
        }
    ...
    public static function get_top_viewed_photos_from_set($user_id, $photoset_id, $limit = 9) {
        ...
        if (!$resolved_user_id) {
            // Fallback: query top-N by views directly from local DB tables
            // Must consider ALL photos in the album (not just first N by position)
            global $wpdb;
            self::ensure_local_tables();
            $membership_table = $wpdb->prefix . 'fjb_photo_membership';
            $meta_table = $wpdb->prefix . 'fjb_photo_meta';
            $rows = $wpdb->get_results($wpdb->prepare(
                "SELECT m.photo_id, COALESCE(pm.views, 0) AS views
                 FROM {$membership_table} m
                 LEFT JOIN {$meta_table} pm ON m.photo_id = pm.photo_id
                 WHERE m.photoset_id = %s
                 ORDER BY views DESC
                 LIMIT %d",
                $photoset_id,
                max(1, (int) $limit)
            ...
            if (!empty($rows)) {
                $urls = [];
                foreach ($rows as $row) {
                    $urls[] = 'https://flickr.com/photos/' . rawurlencode($user_id) . '/' . $row->photo_id . '/';
                }
                return $urls;
            }
            return [];
    ...
    public static function get_photoset_photos($user_id, $photoset_id, $page = 1, $per_page = 50) {
        ...
        $resolved_user_id = self::resolve_user_id($user_id);
        if (!$resolved_user_id) {
            // Fallback: try local membership table when user ID can't be resolved (e.g. no API key)
            $local = self::get_photoset_photos_locally($photoset_id, $user_id, $page, $per_page);
            if (null !== $local) {
                return $local;
            }
            return self::empty_photoset_result($page);
        ...
        $api_key = self::get_api_key();
        if (empty($api_key)) {
            // Fallback: try local membership table when API key is unavailable
            $local = self::get_photoset_photos_locally($photoset_id, $user_id, $page, $per_page);
            if (null !== $local) {
                return $local;
            }
