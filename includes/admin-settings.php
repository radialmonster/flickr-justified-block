...
class FlickrJustifiedAdminSettings {
    ...
    private static function encrypt_api_key($api_key) {
        ...
        // Use WordPress salt for encryption key, hashed to exactly 32 bytes for AES-256-CBC
        // (OpenSSL 3.x rejects keys that aren't exactly 32 bytes)
    ...
    private static function decrypt_api_key($encrypted_api_key) {
        ...
        // Hash salt to exactly 32 bytes for AES-256-CBC
        // (OpenSSL 3.x rejects keys that aren't exactly 32 bytes)
        $key = hash('sha256', wp_salt('auth'), true);
    ...
    public static function api_key_callback() {
        ...
        echo '<p class="description"><strong>' . __('Note:', 'flickr-justified-block') . '</strong> ' . __('Testing the key does not save it. Click "Save Settings" at the bottom of the page to save your key.', 'flickr-justified-block') . '</p>';
        ...
        if (!empty($encrypted_api_key)) {
            if (!empty($decrypted_key) && strlen($decrypted_key) >= 4) {
                ...
                echo '<p class="description" style="color: #d63638;">[ERROR] ' . __('A saved key exists but could not be decrypted. Please re-enter your API key and click Save Settings.', 'flickr-justified-block') . '</p>';
            }
        }
    }
